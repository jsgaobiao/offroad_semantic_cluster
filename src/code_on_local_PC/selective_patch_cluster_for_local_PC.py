...
Author: Gao Biao
Date: 2020-05-19
LastEditTime: 2021-06-17 19:30:10
...
def predict_video(args, cluster_model, rbf_threshold):
    ...
    # 把每一帧所有patch计算得到的rbf响应保存到excel中
    rbf_response_file = os.path.join(args.result_path.replace("cluster_results", "."),'rbf_response.csv')
    cluster_label_file = os.path.join(args.result_path.replace("cluster_results", "."),'cluster_label.csv')
    entropy_file = os.path.join(args.result_path.replace("cluster_results", "."),'entropy.csv')
    tot_patches = 0
    selected_patches =0 
    wait_time = 0
    # with torch.no_grad():
    with open(entropy_file, 'w') as ef:
        with open(cluster_label_file, 'w') as cf:
            with open(rbf_response_file, 'w') as f:
                ################  读入视频帧  #################
                while True:
                    ...
                    f.write("{},".format(frame_id))
                    cf.write("{},".format(frame_id))
                    ef.write("{},".format(frame_id))
                    ...
                    for i in range(features_for_save.shape[0]):
                        ...
                        rbf_response = 1-rbf_dis    # 值域是0-1 这样离聚类中心越近，值越小
                        ...
                        for _k in range(args.kmeans):
                            # 计算到聚类中心的欧几里得距离
                            euclidean_dis = np.linalg.norm(features_for_save[i, 2:] - cluster_model.cluster_centers_[_k])
                            rbf_dis = np.exp(-euclidean_dis**2/(2*_sigma*_sigma))
                            e_rbf_response = 1-rbf_dis
                            pseudo_rbf_prob[_k] = e_rbf_response
                        pseudo_rbf_prob = my_softmax(pseudo_rbf_prob)
                        entropy_rbf = my_entropy(pseudo_rbf_prob)
                        ...
                        if (rbf_response < rbf_threshold):
                            ...
                            selected_patches += 1
                        f.write("{},".format(rbf_response))
                        ef.write("{},".format(entropy_rbf))
                        cf.write("{},".format(_patch_label[0]))
                        time2 = time.time()
                    ...
                    if (show_img):
                        ...
                        cv2.putText(merged_img, "frame {}".format(frame_id), (50,150), cv2.FONT_HERSHEY_COMPLEX, 3, (0,0,255), 10)
                        videoWriter.write(cv2.resize(merged_img, video_size))
                        cv2.imshow("show", cv2.resize(merged_img, video_size))
...
def calcRiskCoverageOfAnchors(args, cluster_method, cluster_model, anchor_feature_list, type_of_anchor_feature_list, selected_coverage=0.8):
    ...
    # 对训练集risk-coverage曲线进行采样
    with open(os.path.join(args.result_path.replace("cluster_results", "."),"trainset_risk_coverage.csv"), 'w') as f:
        for cov in range(1, 100):
            rbf_risk = risk_coverage[int(cov * 0.01 * len(risk_coverage))][0]
            f.write("{},".format(rbf_risk))
...
def main():# 供直接运行本脚本
    ...
    rbf_threshold = calcRiskCoverageOfAnchors(args, cluster_method, cluster_model, anchor_feature_list, type_of_anchor_feature_list, selected_coverage=0.85)
    print("rbf_threshold:{}".format(rbf_threshold))
