...
def predict_video_frame(args, model, k_means_model):
    '''
        [处理指定视频帧] 预测视频帧中每个patch的分割结果
    '''    
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            cv2.imwrite(os.path.join(args.result_path.replace("cluster_results", "case_study"), str(frame_id)+"_pred_all.png"), full_img)      
            cv2.imwrite(os.path.join(args.result_path.replace("cluster_results", "case_study"), str(frame_id)+"_pred_all.mask.png"), _patch_mask)        
            # 将计算好的patch对应的feature保存到文件里
            ...
            # print info
            print('Save video fine segmentation: [{0}] {1}'.format(frame_id, os.path.join(args.result_path.replace("cluster_results", "case_study"), str(frame_id)+"_pred_all.png")))
            # 只处理1帧就退出 DEBUG
            break
