'''
    读取每帧的uncertainty(entropy)分布并: 1.把uncertainty(entropy)可视化到视频图像上  2.绘制每帧uncertainty(entropy)的直方图
'''
import numpy as np
import os
...
if __name__ == "__main__":
    ...
    all_avg_entropy_data = []
    all_avg_entropy_data_frm_id = []
    ...
    entropyVideoWriter = cv2.VideoWriter(os.path.join(args.result_path,'entropy_euclidean.avi'), cv2.VideoWriter_fourcc('M','J','P','G'), int(30), video_size)
    all_frm_entropy_dict = np.load(os.path.join(args.result_path, "uncertainty_txt", "all_frame_entropy_euclidean_dict.npy"), allow_pickle=True).item()
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            #################### 读入保存好的uncertainty(entropy)分布 ###################
            entropy_list = all_frm_entropy_dict[frame_id]
            avg_entropy_data = entropy_list
            all_avg_entropy_data.append(avg_entropy_data)
            all_avg_entropy_data_frm_id.append(frame_id)
            
            #################### 绘制uncertainty(entropy)直方图 ########################
            # 对uncertainty的值进行归一化到0-255
            u_min = min(1.70, entropy_list.min()) #min(1.64, entropy_list.min())
            u_max = max(1.79, entropy_list.max()) #max(1.79, entropy_list.max())
            print('Entropy range of frame{0}: [{1}, {2}]'.format(frame_id, entropy_list.min(), entropy_list.max()))
            fig, ax = plt.subplots(figsize=(5,5))
            ax.set_ylim(0, 1)
            # 用weights加权数据，使得y轴为百分比
            ax.hist(avg_entropy_data, bins=10, range=(u_min,u_max), weights=np.ones(len(avg_entropy_data))/len(avg_entropy_data), log=False)
            ...
            for i in range(full_img.shape[0]//2, full_img.shape[0]-pred_res//2+1, pred_res):
                for j in range(pred_res//2, full_img.shape[1]-pred_res//2+1, pred_res):
                    idx += 1
                    u_color = int(min(255, max(0, (avg_entropy_data[idx] - u_min) / (u_max - u_min) * 255)))
                    ...
                    if frame_id in anchor_dict.keys():
                        for _p in anchor_dict[frame_id]:
                            ...
                            if abs(i-_p[0])<patch_size//2 and abs(j-_p[1])<patch_size//2:
                                anchor_uncertainty_dict[_p[2]].append(avg_entropy_data[idx])
            ...
            merged_uncertainty_img = cv2.addWeighted(full_img, 0.85, _uncertainty_mask, 0.3, 0)
            ...
            # cv2.imwrite(os.path.join(args.result_path, str(frame_id)+"_uncertainty_and_hist__entropy.png"), merged_uncertainty_and_hist_img)
            if frame_id % 1000 == 0:
                # 将不确定性文件保存为txt，方便C++程序读取
                np.savetxt(os.path.join(args.result_path, "uncertainty_txt", "all_frame_uncertainty__entropy_euclidean.txt"), all_avg_entropy_data, fmt="%.6f",delimiter="\t")
                np.savetxt(os.path.join(args.result_path, "uncertainty_txt", "all_frame_uncertainty_frameID__entropy_euclidean.txt"), all_avg_entropy_data_frm_id, fmt="%d",delimiter="\n")
                np.save(os.path.join(args.result_path, "uncertainty_txt", "anchor_uncertainty_dict__entropy.npy"), anchor_uncertainty_dict)
            entropyVideoWriter.write(merged_uncertainty_and_hist_img)
            plt.close(fig)
